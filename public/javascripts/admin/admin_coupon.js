function deleteCategory(a){
    ...
    document.getElementById('deleteCategoryBtn').addEventListener('click',(e)=>{
        e.stopPropagation();
        fetch(`http://localhost:4000/admin/coupon/deletecoupon/${a}`)
        .then((a)=>{
            return a.json()
        })
        .then((a)=>{
...
function categoryStatus(a,b){
    ...
    document.getElementById('publishStatus').addEventListener('click',(e)=>{
        ...
        fetch('http://localhost:4000/admin/coupon/status',{
            method: 'POST',
            headers: {
...
async function productEdit(a){
    let b=await fetch(`http://localhost:4000/admin/coupon/edit/${a}`)
    b=await b.json()
    ...
    document.querySelectorAll('.editcategorybtnsub').forEach((a,i)=>{
         a.formAction=`/admin/coupon/edit/`+b.data._id+'/'+i
    })
